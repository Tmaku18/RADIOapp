...
export default function SearchPage() {
  ...
  const loadExplore = useCallback(async (append: boolean, cursor?: string | null) => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Explore creators and posts.</p>
      </div>
        ...
        <div className="space-y-6">
          ...
          {results.people.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-sm font-semibold uppercase tracking-wide">People</h2>
                ...
                {results.people.map((person) => (
                  ...
                  >
                    {person.avatarUrl ? (
                      ...
                      <div className="w-11 h-11 rounded-full bg-muted flex items-center justify-center">🎨</div>
                    )}
                    <div className="min-w-0 flex-1">
                      <p className="font-medium truncate">{person.displayName || 'Creator'}</p>
                      {person.headline && (
          ...
          {results.posts.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-sm font-semibold uppercase tracking-wide">Posts</h2>
      ...
      {posts.map((post) => (
        ...
        >
          {post.mediaType === 'video' ? (
            <video src={post.imageUrl} muted playsInline className="absolute inset-0 w-full h-full object-cover" />
          ) : (
            <Image
              src={post.imageUrl}
