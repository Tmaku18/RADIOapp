...
/**
 * Theme switcher for PRO‑NETWORX. The selection is persisted by next-themes
 * (storageKey "pro-networx-theme" set in the root layout), so the choice
 * carries across marketing, auth, and the authenticated app.
 */
export function ThemeToggle({ triggerClassName }: Props) {
  ...
  const activeIcon =
    current === 'light' ? Sun01Icon : current === 'system' ? ComputerIcon : DarkModeIcon;
