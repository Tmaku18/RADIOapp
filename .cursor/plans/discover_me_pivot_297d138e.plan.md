...
overview: Pivot the app from a radio-first product to "Discover Me"—a LinkedIn-like platform for content creators (artists, Catalysts (service providers), listeners) with professional profiles, discovery by service/location, paywalled DMs, job board, and subscription-gated access to the creator network. Radio becomes one tab.
todos: []
isProject: false
---
...
- **Onboarding**: After sign-up, user picks **Listener**, **Artist**, or **Catalyst (service provider)**. Store in `users.role` and drive different default views and features.
...
No new role value required; ensure onboarding writes `role` and, for Catalysts (service providers), writes to `service_providers` + `service_provider_types` (or equivalent).
...
- **Unified profile model** (conceptually like LinkedIn):
  ...
  - **Skills / services**: for artists—genres, instruments; for Catalysts (service providers)—service types (from DB) plus optional skill tags.
  - **Portfolio / experience**: for artists—tracks, releases, links; for Catalysts (service providers)—portfolio items (you have `provider_portfolio_items`: image/audio, title, description). Consider an optional “experience” or “projects” section (e.g. “Mixed for Artist X”, “Shot video for Y”) if you want it more LinkedIn-like.
...
- **Profile UX**: Single profile page per user that adapts by role (artist vs Catalyst (service provider) vs listener). Listeners can have a lighter profile (photo, name, maybe short bio) or minimal.
...
- **Browse people** (artists + Catalysts (service providers)) with:
...
- **Browsing**: Catalysts (service providers) (and optionally artists) browse open jobs; can apply or “express interest” (add `service_request_applications` or link messages to `request_id`).
...
- **New**: Introduce an **“Artist plan” upgrade** or **“Creator network” / “Pro” subscription** that:
  - Unlocks **access to the creator network**: browse Catalysts (service providers), view full profiles, and **use direct messaging**.
  - May still grant radio credits as today, or separate “radio” vs “network” plans—your choice.
- **Gating**: Backend checks subscription (and optionally role) before: listing Catalysts (service providers) in discovery, opening a conversation, sending a message, or viewing full profile. Frontend shows upgrade CTA when not subscribed.
...
2. **Onboarding**: Role selection (listener / artist / Catalyst (service provider)); Catalyst (service provider) subtypes.
