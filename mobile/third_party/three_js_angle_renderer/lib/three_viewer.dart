...
class ThreeJS with WidgetsBindingObserver{
  ...
  Future<void> initPlatformState() async {
    if(texture == null){
      // Force the device-native GLES path (useAngle: false) on all targets.
      // flutter_angle 0.4.0 routes physical Android devices to a native ANGLE
      // path whose libEGL_angle.so / libGLESv2_angle.so are NOT shipped, so 3D
      // renders blank on real hardware while emulators (isEmulator -> legacy
      // path) work. Pinning useAngle:false makes every device use the same
      // working path. See FlutterAngle.init([useDebugContext, useAngle]).
