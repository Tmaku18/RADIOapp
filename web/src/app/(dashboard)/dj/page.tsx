...
export default function LiveDjPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const djs = Array.isArray(data?.sessions)
        ? data.sessions.filter((s) => s.hostRole === 'dj')
        : [];
      setSessions(djs);
    } catch {
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Tune into a live DJ broadcasting right now — audio and video, in real time.
          </p>
        </div>
        {canHost && (
          <Button
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-16 text-center space-y-3">
            ...
            <p className="text-lg font-medium text-foreground">No DJ is live right now</p>
                ...
                <CardContent className="p-0">
                  ...
                  <div className="p-4 flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-sm text-muted-foreground truncate">{s.title || 'Live DJ set'}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
