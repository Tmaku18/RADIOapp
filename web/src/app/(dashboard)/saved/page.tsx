...
export default function SavedPostsPage() {
  ...
  const loadInitial = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await discoveryApi.listBookmarks({ limit: PAGE_SIZE });
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const res = await discoveryApi.listBookmarks({ limit: PAGE_SIZE, cursor });
  ...
  const handlePostUpdate = useCallback((next: DiscoverFeedPost) => {
    // Drop posts that were just un-saved so the list stays accurate.
    if (next.bookmarkedByMe === false) {
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Posts you bookmarked to revisit later.</p>
        </div>
      </div>
