...
function ShapeStage({ progressRef }: { progressRef: MutableRefObject<number> }) {
  const caterpillar = useRef<THREE.Group>(null);
  const cocoon = useRef<THREE.Mesh>(null);
  const butterflyL = useRef<THREE.Mesh>(null);
  const butterflyR = useRef<THREE.Mesh>(null);
  ...
  useFrame((state, dt) => {
    ...
    const stage1 = THREE.MathUtils.smoothstep(p, 0.0, 0.22);
    const stage2In = THREE.MathUtils.smoothstep(p, 0.18, 0.4);
    const stage2Out = THREE.MathUtils.smoothstep(p, 0.45, 0.6);
    const stage3 = THREE.MathUtils.smoothstep(p, 0.55, 0.78);
    const stage3Out = THREE.MathUtils.smoothstep(p, 0.78, 0.9);
    const stage4 = THREE.MathUtils.smoothstep(p, 0.82, 1.0);
    ...
    if (cocoon.current) {
      const mat = cocoon.current.material as THREE.MeshStandardMaterial;
      cocoon.current.scale.setScalar(
        THREE.MathUtils.lerp(0.1, 1, stage2In) * (1 - stage2Out * 0.6),
      );
      mat.opacity = stage2In * (1 - stage2Out);
    ...
    if (butterflyL.current && butterflyR.current) {
      ...
      const lMat = butterflyL.current.material as THREE.MeshStandardMaterial;
      const rMat = butterflyR.current.material as THREE.MeshStandardMaterial;
      lMat.opacity = op;
      rMat.opacity = op;
    ...
    if (diamond.current) {
      const mat = diamond.current.material as THREE.MeshStandardMaterial;
      mat.opacity = stage4;
