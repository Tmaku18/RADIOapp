...
const btnPrimary =
  'group inline-flex items-center gap-3 px-7 py-4 rounded-full bg-cyan-400 text-black font-dim-mono text-[12px] tracking-[0.25em] uppercase font-bold glow-cyan hover:bg-white transition-colors';
const btnOutline =
  'inline-flex items-center gap-3 px-7 py-4 rounded-full border border-white/20 text-white font-dim-mono text-[12px] tracking-[0.25em] uppercase hover:border-pink-400 hover:text-pink-400 transition-colors';
const btnOutlineCyan =
  'inline-flex items-center gap-3 px-7 py-4 rounded-full border border-white/20 text-white font-dim-mono text-[12px] tracking-[0.25em] uppercase hover:border-cyan-400 hover:text-cyan-300 transition-colors';
...
function ProLandingHeroCta({ variant }: { variant: ProNetworxLandingVariant }) {
  const { user, loading } = useAuth();
  const directoryHref = variant === 'app' ? '/pro-networx/home' : '/pro-directory';

  if (variant === 'app' && !loading && user) {
    return (
      <div className="mt-10 flex flex-wrap items-center gap-4">
        <Link href={APP_HOME} data-testid="pro-hero-open-app" className={btnPrimary}>
          <Sparkles className="w-4 h-4" />
          Enter Pro-Networx
          <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
        </Link>
        <Link href="/pro-networx/search" className={btnOutline}>
          Browse the feed
        </Link>
        <Link href={directoryHref} data-testid="pro-hero-directory" className={btnOutlineCyan}>
          Explore directory
        </Link>
      </div>
    );
  ...
  if (variant === 'app') {
    return (
      <div className="mt-10 flex flex-wrap items-center gap-4">
        <Link href={SIGNUP_REDIRECT} data-testid="pro-hero-create-profile" className={btnPrimary}>
          <Sparkles className="w-4 h-4" />
          Create your profile — free
          <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
        </Link>
        <Link href={LOGIN_REDIRECT} className={btnOutline}>
          Log in
        </Link>
        <Link href={directoryHref} data-testid="pro-hero-directory" className={btnOutlineCyan}>
          Explore directory
  ...
  return (
    <div className="mt-10 flex flex-wrap items-center gap-4">
      <a href={PRO_APP_URL} data-testid="pro-hero-open-app" className={btnPrimary}>
        <Sparkles className="w-4 h-4" />
        Open the app
        <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
      </a>
      <Link href="/signup" data-testid="pro-hero-create-profile" className={btnOutline}>
        Create your profile — free
      </Link>
      <Link href="/pro-directory" data-testid="pro-hero-directory" className={btnOutlineCyan}>
        Explore directory
      </Link>
    </div>
  );
      ...
      <section className="relative min-h-[calc(100vh-7rem)] flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-10 py-32 w-full">
          <div className="max-w-3xl">
            ...
            <Reveal delay={0.85}>
              <ProLandingHeroCta variant={variant} />
            </Reveal>
      ...
      >
        ...
        <Reveal delay={0.3}>
          <p className="mt-10 text-center text-white/50 font-dim-mono text-[11px] tracking-[0.2em]">
            ...
            <Link href={loginFooterHref} className="text-cyan-300 hover:text-pink-400">
